...
using Knyaz.Optimus.Scripting.Jint.Internal.Tools;
...
namespace Knyaz.Optimus.Scripting.Jint.Internal
{
	...
	internal class DomConverter : IObjectConverter
	{
		...
		public bool TryConvert(object value, out JsValue result)
		{
			...
			if (value is IRawJson json)
			{
				try
				{
					result = engine.Json.Parse(null, new[] {new JsValue(json.JsonString)});
				}
				catch
				{
					result = JsValue.Null;
			...
			//Create wrapper for IReadOnlyList<>
			var valueType = value.GetType();
			var iFace = valueType.GetInterfaces().Where(x => x.IsGenericType).FirstOrDefault(x =>
			{
				var genericArgs = x.GetGenericArguments();
				if (genericArgs.Length != 1)
					return false;
				var roType = typeof(IReadOnlyList<>).MakeGenericType(x.GenericTypeArguments);
				return roType.IsAssignableFrom(x);
			});
			
			if (iFace != null)
			{
				var genericArgs = iFace.GenericTypeArguments;
				var type = typeof(ReadonlyListAdapter<>).MakeGenericType(genericArgs);
				var prototype = GetPrototype(valueType);
				result = (ArrayInstance) Activator.CreateInstance(type, engine, value, prototype);
				_cache[value] = result;
				return true;
			}
			...
			{
				var wrapper = new ClrObject(engine, value, GetPrototype(value.GetType()), this); 
				result = new JsValue(wrapper);
				_cache[value] = result;
				return true;
			}
			...
			if (typeof(Delegate).IsAssignableFrom(targetType))
            {
                ...
                else
                {
                    ...
                    if (generic != null)
                    {
                        ...
                        else if (generic == typeof(Action<>) || generic == typeof(Func<,>))
                        {
                        ...
                        else if (generic == typeof(Action<,>)) //todo: two and more arguments callback
                        {
		...
		public static void DefineStatic(ObjectInstance jsObject, Type type)
		{
				...
				.Where(p => p.GetCustomAttribute<JsHiddenAttribute>() == null))
			{
				...
				var jsValue = JsValue.FromObject(jsObject.Engine, clrValue);

				jsObject.FastAddProperty(name, jsValue, false, false, false);
			}
		}
		public static bool Convertible(Type valType, Type parType)
		{
			if (valType == null)
